...
function ClientProjectPage({ 
  ...
  const [repoInfo, setRepoInfo] = useState<any>(initialRepoInfo);
  ...
  useEffect(() => {
    if (project) {
      const [ownerVal, repoVal] = project.github_full_slug?.split('/') || [];
      setOwner(ownerVal);
      setRepo(repoVal);
      
      // Only sync with GitHub, no need to fetch data again
      if (ownerVal && repoVal) {
        syncProjectWithGitHub(Number(project.id));
      }
    } else {
      router.push('/404');
    }
